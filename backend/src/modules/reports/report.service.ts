...
export async function getReportSummary() {
  ...
  return { patients, visits, waitingQueues, medicinesLowStock };
}

function getPeriod(startQuery: unknown, endQuery: unknown) {
...
export async function getDetailedReport(startQuery: unknown, endQuery: unknown) {
  const { start, end } = getPeriod(startQuery, endQuery);
...
export async function getExportSummary(startQuery: unknown, endQuery: unknown) {
  const { start, end } = getPeriod(startQuery, endQuery);
